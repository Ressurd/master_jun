		...
		for (int i = 0; i < 50; i++) {
			...
			try {
				...
				if (bSize > 0) {
					...
					stanNow = BigDecimal.valueOf(Double.parseDouble(chartService.getPrice(coinNmB, 3).get("trade_price").toString()));
					temp_high = BigDecimal.valueOf(Double.parseDouble(tenIchimokuMap.get("high_prereqSpan").toString()));
				
					if(type.equals("10")) {
						/*손절*/
						if (stanNow.compareTo((BigDecimal) tenIchimokuMap.get("high_prereqSpan"))==-1&&stanNow.compareTo((BigDecimal) twentyichimokuMap.get("low_prereqSpan"))==-1) {
							sellCoin = true;
							cause="손절";
						}
						
						if (stanNow.compareTo((BigDecimal) tenIchimokuMap.get("high_prereqSpan"))==1&&stanNow.compareTo((BigDecimal) fiveIchimokuMap.get("high_prereqSpan"))==-1) {
							sellCoin = true;
						...
						if(temp_high.compareTo((BigDecimal) tenIchimokuMap.get("high_prereqSpan")) != -1 && stanNow.compareTo(ma120) == 1) {
							sellCoin = true;
							cause="120일선";
						}
							
						if (temp_high.compareTo((BigDecimal) tenIchimokuMap.get("high_prereqSpan")) == -1 && stanNow.compareTo((BigDecimal) twentyichimokuMap.get("low_prereqSpan"))==1) {
							sellCoin = true;
							cause="20구름대 아래";
						}
					}else if(type.equals("20")){
						/* 손절부터 확인 */
						if (stanNow.compareTo((BigDecimal) twentyichimokuMap.get("high_prereqSpan"))==-1&&stanNow.compareTo((BigDecimal) tenIchimokuMap.get("low_prereqSpan"))==-1) {
							sellCoin = true;
							cause="손절";
						}
						
						if (stanNow.compareTo((BigDecimal) twentyichimokuMap.get("high_prereqSpan"))==1&&stanNow.compareTo((BigDecimal) tenIchimokuMap_s.get("high_prereqSpan"))==-1) {
							sellCoin = true;
					...
					if (sellCoin) {
						...
						Double dtmp = Double.parseDouble(stanNow.divide(
								BigDecimal.valueOf(Double.parseDouble(buyTmpMap.get("buyValue"))), 6,
								BigDecimal.ROUND_DOWN).toPlainString());
						System.out.println("손익 : " + dtmp);
						last =BigDecimal.valueOf(dtmp).subtract(BigDecimal.valueOf(1)).multiply((BigDecimal.valueOf(0.1))).add(last).setScale(6,RoundingMode.DOWN);
				...
				stanNow = BigDecimal.valueOf(Double.parseDouble(chartService.getPrice(coinNm, 3).get("trade_price").toString()));
				stanHigh = BigDecimal.valueOf(Double.parseDouble(chartService.getPrice(coinNm, 3).get("high_price").toString()));
				stanlow = BigDecimal.valueOf(Double.parseDouble(chartService.getPrice(coinNm, 3).get("low_price").toString()));
				...
				if (stanlow.compareTo(ma180) != -1) {
					/* 10일목 */ 

					if (temp_low.compareTo((BigDecimal) twentyichimokuMap.get("high_prereqSpan")) == 1) {
						if (temp_low.compareTo((BigDecimal) ThreeIchimokuMap.get("high_prereqSpan")) == 1
								&& temp_high.compareTo((BigDecimal) tenIchimokuMap_s.get("high_prereqSpan")) == 1) {
							if (ma60.compareTo(ma120) == 1) {
								if (stanHigh.compareTo(temp_high) == 1 && stanlow.compareTo(temp_high) == -1) {
									System.out.println("-------매수타점 발생 알림-------");
									System.out.println(coinNm + " 10"); // System.out.println(jsArray.toString());
									System.out.println("매수가 : " + temp_high.toPlainString());
									System.out.println("현재봉 고가 : " + stanHigh.toPlainString());
									System.out.println("일목HIGH : " + temp_high.toPlainString());
									Map<String, String> buyMap = new HashMap<String, String>();
									buyMap.put("market", coinNm);
									buyMap.put("buyValue", temp_high.toPlainString());
									buyMap.put("type", "10");
									buyList.add(buyMap);
					...
					/* 20일목 */
					if (temp_high.compareTo((BigDecimal) twentyichimokuMap.get("low_prereqSpan")) == -1) {
						if (((BigDecimal) twentyichimokuMap.get("low_prereqSpan"))
								.compareTo((BigDecimal) ThreeIchimokuMap.get("high_prereqSpan")) == 1
								&& temp_high.compareTo((BigDecimal) ThreeIchimokuMap.get("low_prereqSpan")) == -1) {
							if (ma120.compareTo(ma60) == 1) {
								if (stanHigh.compareTo(((BigDecimal) twentyichimokuMap.get("high_prereqSpan"))) != -1
										&& stanlow.compareTo(
												(BigDecimal) twentyichimokuMap.get("high_prereqSpan")) != 1) {
									System.out.println("-------매수타점 발생 알림-------");
									System.out.println(coinNm + " 20");
									// System.out.println(jsArray.toString());
									System.out.println("매수가 : "
				...
				BigDecimal sixtyltemp =  BigDecimal.valueOf(Double.parseDouble(twentyichimokuMap.get("low_prereqSpan").toString()));
				
				/* 60로우 */
				if (temp_high.compareTo((BigDecimal) twentyichimokuMap.get("low_prereqSpan")) == -1 &&
						sixtyltemp.divide(temp_high, 4, BigDecimal.ROUND_DOWN).compareTo(BigDecimal.valueOf(1.005)) != -1 )
				{
					if (ma120.compareTo(ma180)==-1 && ma120.compareTo(bb60h) == 1 && temp_high.compareTo(ma60) != -1) {
